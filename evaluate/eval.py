...
def run_evaluation_rag(temperature=0.0, top_p=0.95):
    ...
    def run_hp_qa_dataset():
        ...
        for i in range(0, len(ds), 20):
            print(f"Processing batch {i // 20 + 1} of {len(ds) // 20}")
            ground_truths = []
            questions = []
            for j in range(i, i + 20):
                context = ds[j]['context']
                questions.append(context + "\n\n" + ds[j]['question'])
                ground_truths.append(ds[j]['answer'])
        ...
        results = evaluate(dataset=evaluation_dataset, metrics=[LLMContextRecall(), Faithfulness(), FactualCorrectness()], llm=evaluator_llm)
